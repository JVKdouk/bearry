...
import { durationLabel, LIFE_AREA_COLOR, PRIORITY_COLOR, PRIORITY_LABEL } from "@/lib/format";
...
export function TaskCard({
  ...
  function completeTask() {
    ...
    timers.current.push(
      ...
      window.setTimeout(() => setPhase("slide"), 500),
      // …and once it's gone, the gap collapses so the rest settle upward…
      window.setTimeout(() => setPhase("collapse"), 500 + 200),
      // …and only then is it actually completed (which unmounts it here).
      window.setTimeout(() => update("block", todo.id, { status: "done" }), 500 + 200 + 100),
    );
  }
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
                      ...
                      {todo.estimatedDuration ? (
                        <Pill
                          color={featured ? "#fff" : TEXT.tertiary}
                          tone={featured ? "onAccent" : "soft"}
                        >
